...
class EnterpriseService:
    ...
    async def add(self, dto: AddEnterpriseDto) -> Enterprise:
        return await self.enterprise_repository.add(dto=dto, )

    async def get_by_name(self, name) -> Enterprise:
        # доп бизнес логика
        return await self.enterprise_repository.get_by_name(name=name, )
    ...
    async def update(self,  name_enterprise: str, dto: UpdateEnterpriseDto) -> Enterprise:
        return await self.enterprise_repository.update(name_enterprise=name_enterprise, dto=dto, )

    async def remove(self, name_enterprise: str) -> Enterprise:
        return await self.enterprise_repository.remove(name_enterprise=name_enterprise, )

    async def get_by_estate_name(self, estate_name: str) -> list[Enterprise]:
        return await self.enterprise_repository.get_by_estate_name(estate_name=estate_name, )

    async def get_by_operation_name(self, operation_name: str) -> list[Enterprise]:
        return await self.enterprise_repository.get_by_operation_name(operation_name=operation_name, )

    async def get_by_rectangle(self, extreme_left_diameter_point: str, extreme_right_diameter_point: str) -> list[Enterprise]:
        return await self.enterprise_repository.get_by_rectangle(extreme_left_diameter_point, extreme_right_diameter_point)
